/*
  Code Owners
  - Abigail O'Leary (21363851)
  - Karolina Raczynska (21365865)
  - Varun Anand (21343782)
*/ 
...
#include <math.h>
#include <stdint.h>
#include <x86intrin.h> // mentioned in assignment description
               ...
               int kernel_order)
{
  ...
  #pragma omp parallel for private(w, h, c, x, y) collapse(2)
  for ( m = 0; m < nkernels; m++ ) {
    for ( w = 0; w < width; w++ ) {
      for ( h = 0; h < height; h++ ) {
        double sum = 0.0;
...
int main(int argc, char ** argv)
{
  ...
  long long david_mul_time = (stop_time.tv_sec - start_time.tv_sec) * 1000000L +
  ...
  /* record speed up */
  printf("Student's how many times faster than David's: %.2f times\n", (david_mul_time/(double)mul_time));
  printf("Student's speed up: %.2f%%\n", ((david_mul_time - (double)mul_time)/david_mul_time)*100);
